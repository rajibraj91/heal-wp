    ...
    class Heal_Init
    {
        ...
        */
        ...
        public function theme_setup()
        {
            ...
            register_nav_menus(array(
                'main-menu' => esc_html__('Primary Menu', 'heal'),
                // 'main-menu-2' => esc_html__('Primary Menu Two', 'heal'),
                // 'category-menu' => esc_html__('Category Menu', 'heal'),
                // 'footer-menu' => esc_html__('Footer Menu', 'heal'),
            ));
            ...
            $GLOBALS['content_width'] = apply_filters('heal_content_width', 740);

            //add image sizes
            add_image_size('heal_classic', 750, 400, true);
            add_image_size('heal_grid', 370, 270, true);
            add_image_size('heal_medium', 550, 380, true);
            add_image_size('heal-team-slider-one', 450, 460, true);
            add_image_size('heal-team-single', 636, 675, true); // exact size, hard crop
            add_image_size('heal-event-single', 553, 360, true);
            add_image_size('heal-speaker-thumb', 189, 189, true);
            //---Heal Blog Images 
        ...
        public function theme_widgets_init()
        {
            register_sidebar(array(
                'name' => esc_html__('Sidebar', 'heal'),
                'id' => 'sidebar-1',
                'description' => esc_html__('Add widgets here.', 'heal'),
                'before_widget' => '<div id="%1$s" class="widget gt-single-sideber-widget %2$s">',
                'after_widget' => '</div>',
                'before_title' => '<h3 class="widget-headline style-01 ">',
                'after_title' => '</h3>',
            ));

            register_sidebar(array(
                'name' => esc_html__('Service Sidebar', 'heal'),
                'id' => 'service-sidebar',
                'description' => esc_html__('Add widgets here.', 'heal'),
                'before_widget' => '<div id="%1$s" class=" event-single %2$s">',
                'after_widget' => '</div>',
                'before_title' => '<h3 class="widget-headline style-01 ">',
                'after_title' => '</h3>',
            ));
            register_sidebar(array(
                'name' => esc_html__('Event Sidebar', 'heal'),
                'id' => 'event-sidebar',
                'description' => esc_html__('Add widgets here.', 'heal'),
                'before_widget' => '<div id="%1$s" class=" event-single %2$s">',
                'after_widget' => '</div>',
            ...
            if (heal()->is_heal_core_active()) {
                register_sidebar(array(
                    'name' => esc_html__('Product Sidebar', 'heal'),
                    'id' => 'product-sidebar',
                    'description' => esc_html__('Add widgets here.', 'heal'),
            ...
            // register_sidebar(array(
            //     'name' => esc_html__('Footer Widget Area', 'heal'),
            //     'id' => 'footer-widget',
            //     'description' => esc_html__('Add widgets here.', 'heal'),
       ...
       */
        ...
        public static function load_google_fonts()
        {
            ...
            $body_font['family'] = (isset($body_font['font-family']) && !empty($body_font['font-family'])) ? $body_font['font-family'] : 'Barlow';
            $body_font['font'] = (isset($body_font['type']) && !empty($body_font['type'])) ? $body_font['type'] : 'google';
            $body_font_variant = !empty($body_font_variant) ? $body_font_variant : array(400, 500, 600, 700, 800);
            $google_fonts = array();
        ...
        {
            $theme_version = HEAL_DEV ? time() : heal()->get_theme_info('version');
            $css_ext = '.css';
            // load google fonts
            $enqueue_google_fonts = self::load_google_fonts();
            if (!empty($enqueue_google_fonts)) {
                wp_enqueue_style('heal-google-fonts', esc_url(add_query_arg('family', urlencode(implode('|', $enqueue_google_fonts)), '//fonts.googleapis.com/css')), array(), null);
            }
            $all_css_files = array(
                array(
                    'handle' => 'all.min',
                    'src' => HEAL_CSS . '/all.min.css',
                    'deps' => array(),
                    'ver' => $theme_version,
                    'media' => 'all',
                ),
                array(
                    'handle' => 'animate',
                    'src' => HEAL_CSS . '/animate.css',
                    'deps' => array(),
                    'ver' => '3.5.2',
                    'media' => 'all',
                ),
                array(
                    'handle' => 'bootstrap.min',
                    'src' => HEAL_CSS . '/bootstrap.min.css',
                    'deps' => array(),
                    'ver' => '6.0',
                    'media' => 'all',
                ),
                ...
                array(
                    'handle' => 'swiper',
                    'src' => HEAL_CSS . '/swiper.min' . $css_ext,
                    'deps' => array(),
                    'ver' => $theme_version,
                    'media' => 'all',
                ),   
            ...
            $options = cs_get_option( 'enable_rtl' );

            if ( $options ) {
                // Enqueue RTL CSS
                wp_enqueue_style( 'heal-theme-rtl', HEAL_CSS . '/rtl.css', array(), '1.0.0' );
            }
         
            $all_css_files = apply_filters('heal_theme_enqueue_style', $all_css_files);
            ...
            if (heal()->is_heal_core_active()) {
                if (file_exists(HEAL_DYNAMIC_STYLESHEETS . '/theme-inline-css-style.php')) {
                    require_once HEAL_DYNAMIC_STYLESHEETS . '/theme-inline-css-style.php';
                    require_once HEAL_DYNAMIC_STYLESHEETS . '/theme-option-css-style.php';
                    wp_add_inline_style('heal-style', heal()->minify_css_lines($GLOBALS['heal_inline_css']));
                    wp_add_inline_style('heal-style', heal()->minify_css_lines($GLOBALS['theme_customize_css']));
                }
        ...
        public function load_theme_js()
        {
            ...
            wp_enqueue_script( 'countdown-min', get_template_directory_uri() . '/assets/js/countdown.min.js', array('jquery'), '1.0.0', true );
            wp_enqueue_script( 'donate-range', get_template_directory_uri() . '/assets/js/donate-range.js', array('jquery'), '1.0.0', true );
            wp_enqueue_script( 'isotope-pkgd-min', get_template_directory_uri() . '/assets/js/isotope.pkgd.min.js', array('jquery'), '8.3.2', true );
            wp_enqueue_script( 'jquery-counterup-min', get_template_directory_uri() . '/assets/js/jquery.counterup.min.js', array('jquery'), '2.0.8', true );
            wp_enqueue_script( 'lightcase', get_template_directory_uri() . '/assets/js/lightcase.js', array('jquery'), '1.1.0', true );
            wp_enqueue_script( 'parallax-min', get_template_directory_uri() . '/assets/js/parallax.min.js', array('jquery'), '1.0.0', true );
            wp_enqueue_script( 'swiper-min', get_template_directory_uri() . '/assets/js/swiper.min.js', array('jquery'), '1.0.0', true );
            ...
            wp_enqueue_script( 'heal-main-script', get_template_directory_uri() . '/assets/js/custom.js', array('jquery'), time(), true );
