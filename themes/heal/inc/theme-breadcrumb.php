...
class Heal_Breadcrumb {
	/**
	 * Array of bread Heal_Breadcrumb
	 *
	 * @var array
	 */
	...
	public function __construct( array $options = [] ) {
		...
		$this->defaults['home'] = esc_html__('Home','heal');
		$this->defaults['blog'] = esc_html__('Blog','heal');
	...
	public function theBreadcrumbs() {
		...
		foreach ( $breadcrumbs as $crumb ) {
			...
			if ( $crumb['url'] ) {
                ...
                $html .= sprintf( '<span>%s</span>', wp_kses($crumb['title'],heal()->kses_allowed_html('all')) );
			}
		...
		echo wp_kses($html,heal()->kses_allowed_html('all'));
	}
	...
	public function getBreadcrumbs() {
		...
		$breadcrumbs = apply_filters( 'heal_get_breadcrumbs', $breadcrumbs );
 ...
 * Template Functions.
 */
if ( ! function_exists( 'heal_breadcrumb' ) ) {
	/**
	 ...
	 * @package heal
	 */
	function heal_breadcrumb( $options = [] ) {
		$Crumbs = new Heal_Breadcrumb( $options );
		$Crumbs->theBreadcrumbs();
	}
}

if ( ! function_exists( 'heal_get_breadcrumbs' ) ) {
	/**
	 ...
	 * @return array
	 */
	function heal_get_breadcrumbs( $options = [] ) {
		$Crumbs = new Heal_Breadcrumb( $options );
 ...
 * Utility functions
 */
if ( ! function_exists( 'heal_breadcrumb_array_insert' ) ) {
	/**
	 ...
	 * @return array
	 */
	function heal_breadcrumb_array_insert( array $array, $element, int $position ) {
