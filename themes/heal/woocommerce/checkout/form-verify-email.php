...
<form name="checkout" method="post" class="woocommerce-form woocommerce-verify-email" action="<?php echo esc_url( $verify_url ); ?>">
	...
	if ( $failed_submission ) {
		wc_print_notice( esc_html__( 'We were unable to verify the email address you provided. Please try again.', 'heal' ), 'error' );
	}
	?>
	<p>
		<?php
		printf(
			...
			esc_html__( 'To view this page, you must either %1$slogin%2$s or verify the email address associated with the order.', 'heal' ),
	...
	<p class="form-row">
		<label for="email"><?php esc_html_e( 'Email address', 'heal' ); ?>&nbsp;<span class="required">*</span></label>
		<input type="email" class="input-text" name="email" id="email" autocomplete="email" />
	</p>

	<p class="form-row">
		<button type="submit" class="theme-btn woocommerce-button button <?php echo esc_attr( wc_wp_theme_get_element_class_name( 'button' ) ); ?>" name="verify" value="1">
			<?php esc_html_e( 'Verify', 'heal' ); ?>
		</button>
	</p>
</form>
